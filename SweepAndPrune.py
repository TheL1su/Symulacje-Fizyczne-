...
scene.width = 900
scene.height = 700
scene.title = "Sweep and Prune - 3D Box Collision"
...
def sweep_and_prune(boxes):
    intervals = [(box, box.AABB()["min"][0], box.AABB()["max"][0]) for box in boxes]
    ...
    for i in range(len(boxes)):
        ...
        for j in range(i+1,len(boxes)):
            box2 = intervals[j][0]
            cords2 = intervals[j]
            if(cords1[2] <= cords2[1]):
                break
            if(find_collisions(box1,box2)):
                collisions.add(box1)
                collisions.add(box2)
...
while True:
    ...
    collisions = sweep_and_prune(boxes)
